...
function fifedu() {
  (function () {
    const targetURL =
      "https://moral.fifedu.com/kyxl-app-challenge/evaluation/submitChallengeResults";
        ...
        if (typeof data === "string") {
          try {
            ...
            tmptext
              ...
              .split("&")
              .forEach(
                (item) => (tmpjson[item.split("=")[0]] = item.split("=")[1])
              );
            ...
            tmptext +=
              "&resultJson=" +
              encodeURIComponent(JSON.stringify(tmpjson.resultJson));
            console.log(tmptext);
            data = tmptext;
          } catch (e) {
            console.log("出错了");
...
function getallVideos() {
  NCVTpost(
    "Student/Course/getCoursesCanApply.html",
    {
      p: allVideos_p,
    ...
    (succ) => {
      ...
      if (tmp[1].length === 0) {
        return mui.confirm(
          `获取完毕，要入库共${allVideos.length}个课程吗？`,
          "提示",
          ["取消", "确认"],
          function (e) {
            if (e.index === 1) {
...
function checkVideos() {
  allVideos.map((item) => {
    NCVTpost(
      "Student/Course/courseRegister.html",
      {
...
function getallxxzVideos() {
  NCVTpost(
    "Student/My/myCourse_xxz.html",
    {
      p: allxxzVideos_p,
      maxID: allxxzVideos_maxID,
    },
    (succ) => {
      let tmp = JSON.parse(succ);
      ...
      if (tmp[1].length === 0) {
        mui.confirm(
          `获取完成，共${allxxzVideos.length}个课程，要开始学习吗？因脚本与页面共存，所以请勿对面进行刷新和回退等操作`,
          "提示",
          ["取消", "确认"],
          function (e) {
            if (e.index === 1) {
...
function start_allxxzVideos() {
  ...
  allxxzVideos.map((item) => {
    NCVTpost(
      "Student/Course/getSecondsAndRemmberStartTime.html",
      {
        courseID: item.courseID,
      },
      (succ) => {
...
function xxzpages_dom() {
  ...
  allxxzVideos.map((item) => {
    mui(
      "#ajaxbox1"
    )[0].innerHTML += `<a class="mui-clearfix" id="xxzdom_${item.courseID}">
...
function stop_allxxzVideos() {
  ...
  allxxzVideos.map((item, index) => {
    NCVTpost(
      "Student/Course/remmberStudyTime.html",
      {
        courseID: item.courseID,
...
function get_allwpjvideos() {
  NCVTpost(
    ...
    (succ) => {
      ...
      if (tmp[1].length === 0) {
        mui.confirm(
          `获取完成，共${allwpjVideos.length}个课程，要开始评价吗？`,
          "提示",
          ["取消", "确认"],
          function (e) {
            if (e.index === 1) {
...
function Ncvt_getxxzvideo_start() {
  ...
  mui(
    "#tab2"
  )[0].innerHTML = `<button type="button" class="mui-btn mui-btn-primary" id="finish_botton" style="width: 100%;vertical-align: middle;">未评价</button>`;
    ...
    if (newStr == "#/dekaronData/dekaronMain") {
      ...
      if (tmpnum != null) {
        ...
        if (arr.length < 5 || arr.length > 5) {
          window.alert(
            "分数不合规，请刷新页面后重新输入，设定分数不成功修改不会生效"
          );
        }
...
if (window.location.host == "class.bigdata.ncvt.net") {
  if (
    ...
    "https://class.bigdata.ncvt.net/Student/Course/index.html"
  ) {
    Ncvt_getallvideo_start();
  } else if (
    window.location.href ==
    "https://class.bigdata.ncvt.net/Student/My/myCourse.html"
  ) {
    Ncvt_getxxzvideo_start();
  }
  const submit_URL = window.location.href;
  const targetPath =
    "https://class.bigdata.ncvt.net/Student/Activity/apply.html";
