...
function getallVideos() {
	NCVTpost('Student/Course/getCoursesCanApply.html', {
		...
		if (tmp[1].length === 0) {
			return mui.confirm(`获取完毕，要入库共${allVideos.length}个课程吗？`, '提示', ['取消', '确认'], function(e) {
				if (e.index === 1) {
					checkVideos();
				}
			}, 'div')
...
function Ncvt_getallvideo_start() {
	$(document).ready(function() {
		...
		$(window).load(function() {
			mui.confirm('获取要入库的所有的课程？', '提示', ['取消', '确认'], function(e) {
				if (e.index === 1) {
					getallVideos();
				}
			}, 'div')
...
function fiftool_start() {
	...
	if (questionMarkIndex !== -1) {
		...
		if (newStr == "#/dekaronData/dekaronMain") {
			//提示用户输入参数
			let tmpnum = prompt("请输入你要修改成的分数，输入的值为大于0小于100的整数，从左到右依次为：发音、流利度、完整度、最终分数、单词细节分数，用一个空格间隔开");
			if (tmpnum != null) {
				let str = tmpnum;
				let arr = str.split(" ");
				//首先判断输入数据数组长度是否为5
				...
				arr.map((item, index) => {
					if (Number(item) >= 0 && Number(item) <= 100 && Number(item) % 1 === 0) {
						fif_score.push(item);
					} else {
						return;
					}
